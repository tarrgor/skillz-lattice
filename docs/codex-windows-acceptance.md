...
- [ ] Codex can select `verify_implementation` and `research_topic` as custom agents.
...
- [ ] `verify_implementation` runs in a separate agent thread, requests its read-only default, returns complete findings, and does not edit or post. Record any parent runtime override that supersedes the agent's sandbox default.
...
- [ ] `research_topic` receives established project facts as text rather than local paths and performs no observed local reads or writes. Record that this is behavioral validation, not proof of a web-only technical boundary.
